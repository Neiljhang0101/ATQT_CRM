...
> 採用**標準 RFM 8 象限模型**，每個標籤同時參考 R、F、M 三個維度。  
> 每位用戶**恰好一個**主標籤 + 可選疊加標籤。

### 4.1 門檻定義（二元化）

| 維度 | 高（↑）門檻 | 低（↓）門檻 |
|------|-----------|-----------|
| **R** | r ≥ 3（30 天內有活動） | r ≤ 2（超過 30 天未活動） |
| **F** | f ≥ 3（combined ≥ 5） | f ≤ 2（combined < 5） |
| **M** | m ≥ 3（餘額 > 500U 或近30天交易量 > 30萬U） | m ≤ 2（以下） |
...
|------|---|---|---|---------|
| 👑 核心VIP | ↑ | ↑ | ↑ | 最高價值，全力維護 |
| 💰 資金型活躍戶 | ↑ | ↓ | ↑ | 有錢有活動但互動少，加強聯繫頻率 |
| ⚡ 高淨值沉默戶 | ↓ | ↑ | ↑ | 高資產但開始疏遠，優先挽留 |
| 🚨 大戶流失危機 | ↓ | ↓ | ↑ | 高資產全面低落，最緊急挽留 |
| 📊 活躍成長戶 | ↑ | ↑ | ↓ | 互動積極，培養資金規模 |
| 📉 輕倉回訪戶 | ↑ | ↓ | ↓ | 最近有回來但低度參與，引導深入 |
| 🔄 老客低迷戶 | ↓ | ↑ | ↓ | 還在互動但逐漸流失，需再激活 |
| 💤 休眠待喚醒 | ↓ | ↓ | ↓ | 全面沉寂，低優先再行銷 |
...
| 標籤 | 觸發條件 | 行為 |
|------|----------|------|
| 🌟 新手待破蛋 | 距註冊/首入金 ≤ 7 天 | **直接回傳**，不進 8 象限判斷 |
...
| 🔇 社群低互動 | `line_msg_count_7d` < 10 | 灰色 |

> 新手待破蛋也有社群互動標籤，但不觸發「🔇 社群低互動」。

---

## 五、標籤共存規則

- 每位用戶**恰好一個** 8 象限主標籤（或「🌟 新手待破蛋」覆蓋）
- 「🌱 入金未交易」與「💬 社群互動」系列為**疊加標籤**，與主標籤並存
- 社群互動標籤**四者互斥**，只取最高段
- 「🌟 新手待破蛋」不觸發「🔇 社群低互動」
